...
public final class ToadSage {
 ...
 private static void firePulse(ServerPlayerEntity p){ServerWorld w=p.getServerWorld();Vec3d a=p.getEyePos(),d=p.getRotationVec(1).normalize();for(int i=1;i<=16;i++){double z=i*.44,sp=.035*z;Vec3d q=a.add(d.multiply(z));w.spawnParticles(ParticleTypes.DUST,q.x,q.y,q.z,5,sp,sp*.65,sp,.015);w.spawnParticles(ParticleTypes.SMOKE,q.x,q.y,q.z,3,sp*.8,sp*.55,sp*.8,.025);}LivingEntity t=target(p,7,1.25);if(t!=null){t.damage(p.getDamageSources().playerAttack(p),1.5f);AttributeSystem.hit(p,t,Element.FIRE);}}
